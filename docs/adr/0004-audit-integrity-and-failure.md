...
- **상태**: Accepted (2026-08-24 사람 승인 완료)
- **날짜**: 2026-08-21 (승인: 2026-08-24)
- **결정권자**: 아키텍트, 안전 책임자
...
- **되돌림 가능성**: **불가.** 이 결정이 바뀌면 기존 감사 체인 전체가 재검증 불가가 되고,
  저장된 승인 기록이 무효화되며, 골든 벡터 24개를 전부 다시 만들어야 한다.
...
**모든 digest 가 달라진다.** 전역 로케일은 스레드 간 공유되므로 전역 `setlocale()` 에 의존하는 구현을 엄격히 금지한다.

```
숫자 직렬화는 C 로케일 고정 C99 %g 규칙을 따른다 (지수 최소 2자리, 2^53+2 는 십진 정수 9007199254740994).
허용 수단:
  (a) std::to_chars(first, last, v, std::chars_format::general, P)   ← 정의상 로케일 독립
  (b) C 로케일을 명시 고정한 snprintf (_create_locale / newlocale)
```

모든 구현은 기동 시 `ccj::SelfTest()` 의 24개 골든 벡터와 100% 비트 일치해야 한다.
...
```
LP(f₁ … fₙ) = Σᵢ [ field_bytes(fᵢ) ]
H(f₁ … fₙ) = SHA-256( LP(f₁ … fₙ) )

  - 첫 필드는 반드시 도메인 태그 : u32le(len) ‖ UTF-8 tag bytes
  - 문자열 / 바이트열 / CCJ    : u32le(len) ‖ UTF-8 원문 바이트
  - U64 정수                   : raw u64le 8바이트 (별도 길이 접두사 없음, 십진 문자열 금지)
  - Digest                     : raw 32바이트 (별도 길이 접두사 없음, hex 문자열 금지)
  - Optional / 누락 필드       : 항상 빈 문자열 "" (u32le(0)) 로 포함
```

길이 접두사가 있으므로 빈 문자열과 필드 부재가 구분된다. **따라서 모든 프로젝션 슬롯은 생략 없이 고정 개수로 인코딩된다.**
