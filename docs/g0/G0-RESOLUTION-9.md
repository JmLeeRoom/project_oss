...
| **관련 ADR** | `0001-fsm-turn-and-action`, `0004-audit-integrity-and-failure` |

> **읽는 법** — 각 항목은 `[모순] → [선택지] → [확정] → [명세 수정 diff] → [영향]` 순이다.
> **되돌림 불가** 표시가 있는 결정은 나중에 바꾸면 기존 감사 데이터·골든 벡터·승인 기록이 무효가 된다. 먼저 승인하라.
...
| # | 항목 | 확정 결정 | 되돌림 |
| --- | --- | --- | --- |
| ① | G0-01 | ABI **v1.1 단일 기준**. `MAJOR=1 MINOR=1`. §8-2 의 v1.0 시그니처는 명세에서 삭제 | 가능 |
| ② | G0-05 | `operation_digest` 신설(`cogito-operation-v1`). 잠금 키와 멱등 키를 **분리** | **불가** |
| ③ | G0-09 | 무값 성공은 `Error` 로 통일. `Result<void>` 특수화는 제네릭 전용. 예외 정책 확정 | 가능 |
| ④ | G0-23 | **골든표가 권위.** 서술 규칙을 C99 `%g`(지수 최소 2자리)로 정정. **+ 골든표 자체의 오류 1행 발견·정정** | **불가** |
| ⑤ | G0-24 | 보편 규칙 **R0~R4** 완전 정의. `Idle` 은 R1/R2 대상 아님 | 가능 |
| ⑥ | G0-25 | `invoker.hpp` · `ops_log.hpp` · `context_compactor.hpp` 계약 확정 | 가능 |
| ⑦ | G0-26 | 도메인 태그 **9개**(기존 8 + `kOperation`) + projection 표 확정 | **불가** |
| ⑧ | G0-29 | 모드를 숫자 비교하지 않는다. **effect 상한으로 사상 후 최솟값** | 가능 |
| ⑨ | G0-31 | 증가 시점 = `AwaitApproval→Gate`(`Event::Approved`). 상한 1. reset 시점 확정 | 가능 |

**되돌림 불가 3건(②④⑦)은 감사 체인·골든 벡터·승인 결합의 기반이다.** 이 셋을 먼저 승인해야 S1 이 시작될 수 있다.
